...
mean(sampleDia)
```
As you can see, the estimate of $p$ was `r mean(sampleDia)` which is pretty close to the population proportion.
...
p1 <- 
    tibble(t = seq(-4, 4, by = 0.01), dens = dt(t, 30)) %>%
    ggplot(aes(x = t, y = dens)) +
    geom_line() + 
    geom_area(aes(x = ifelse(t > 2.2, t, NA)), fill = 'red', alpha = 0.3) +
    theme_cowplot() +
    labs(x = 'Test statistic', y = '', title = 'One-sided test')
...
p2 <- 
    ...
    geom_area(aes(x = ifelse(t < -2.2, t , NA)), fill = 'red', alpha = 0.3) +
    theme_cowplot() +
    labs(x = 'Test statistic', y = '', title = 'Two-sided test')
...
```{r, echo = F, message = F, warning = F}
tt <- t.test(height ~ intervention, data = pulse)
```
```{r}
t.test(height ~ intervention, data = pulse)
```

We see that the $p$-value of the test is `r tt$p.value` so we are unable to reject the null hypothesis at significance level of $\alpha = 0.05$. Note that we also get the confidence interval. 
